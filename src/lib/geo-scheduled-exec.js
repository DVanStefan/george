...
import { getGeoConfig, normalizeGeoConfig, promptsFromGeoConfig } from "./geo-config-store.js";
...
export async function runScheduledGeoBatch({ cwd = process.cwd(), trigger = "daily" } = {}) {
  ...
  const config = {
    ...
    prompts,
    repeats,
    scheduleTrigger: String(trigger || "daily"),
  ...
  if (guard.skipped) {
    return { ok: true, skipped: true, reason: "active_batch_running" };
  }

  const batchId = `geo-${new Date().toISOString().replace(/[:.]/g, "-")}-${randomUUID().slice(0, 8)}`;
  console.log(`Starting scheduled GEO batch: ${batchId} trigger=${config.scheduleTrigger}`);
  console.log(
  ...
  try {
    ...
    return { ok: true, skipped: false, batchId };
  } catch (err) {
    await updateGeoBatch({
      cwd,
      batchId,
